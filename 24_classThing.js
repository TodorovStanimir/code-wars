        ...
        get: (target, name) => {
          ...
          if (n > 1) {
            ...
            result.each = (callback) => {
              ...
              stringFunc = `${leProp} => ${leProp}.${stringFunc.substring(
                arrowIndex + 3
              )}`;
          ...
          if (cacheProp) {
            finalFunc = new Proxy(finalFunc, {
              apply: (evalFunction, thisArg, argumentsList) => {
                const history = target[cacheProp]
                  ? target[cacheProp]
                  : new Array();
...
console.log(jane.is_a_person); // => true
console.log(jane.is_a_man); // => false
...
console.log(jane.arms[0].hand.fingers.length); // => 5
